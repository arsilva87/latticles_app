...
artigosLattes <- function(XML, area, anos)
{
   ...
   if (class(artpub) != "try-error" & !is.null(artpub)) {
      ...
      artigodat = data.frame(Autor1 = NA, Ano = NA, Titulo = NA, Periodico = NA, ISSN = NA,
         Qualis = NA, Vol = NA, pIni = NA, pFin = NA)
      warning("Nao foram encontrados artigos publicados.") 
   }

   # artigos aceitos ---------------------------------------------------------
   artace <- try(as_list(xml_children(prod))[[grep("ARTIGOS-ACEITOS-PARA-PUBLICACAO", xml_children(prod))]], silent = TRUE)
   if (class(artace) != "try-error" & !is.null(artace)) {
      # loop
      n2 <- length(artace)
      ...
      artigodat2 = data.frame(Autor1 = NA, Ano = NA, Titulo = NA, Periodico = NA, ISSN = NA,
         Qualis = NA, Vol = NA, pIni = NA, pFin = NA)
      warning("Nao foram encontrados artigos aceitos para publicacao.") 
   }
   ...
   if (class(trab) != "try-error" & !is.null(trab)) {
      ...
      trabdat = data.frame(Autor1 = NA, Ano = NA, Titulo = NA, Natureza = NA, Evento = NA, 
         Local = NA, ISBN = NA, Vol = NA, pIni = NA, pFin = NA)   
      warning("Nao foram encontrados trabalhos em eventos.")
   }
   ...
   if (class(livro) != "try-error" & !is.null(livro)) {
      ...
      livrodat <- data.frame(Autor1 = NA, Ano = NA, Titulo = NA, Ed = NA,
         Local = NA, Editora = NA, ISBN = NA, Pags = NA)
      warning("Nao foram encontrados livros publicados.")
   }
   ...
   if (class(cap) != "try-error" & !is.null(cap)) {
      ...
      capdat <- data.frame(Autor1 = NA, Ano = NA, Titulo = NA, Livro = NA, Ed = NA,
         Local = NA, Editora = NA, ISBN = NA, pIni = NA, pFin = NA)
      warning("Nao foram encontrados capitulos de livros publicados.")
   }
   ...
   if (class(orien) != "try-error" & !is.null(orien)) {
      ...
      outorien = NULL
      warning("Nao foram encontradas orientacoes concluidas.")
   }
   ...
   if (class(artpub) != "try-error" & !is.null(artpub)) {
      ...
      artigodat <- metrics <- metrics. <- resumo <- NA
   }
   if (class(artace) != "try-error" & !is.null(artace))  
   ...
   out <- list(nome = nome, atual = datat., metricas = resumo, 
      ...
      livros = summargins(outlivro),
      capitulos = summargins(outcap),
